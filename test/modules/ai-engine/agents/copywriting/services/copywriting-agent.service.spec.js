...
const copywriting_agent_service_1 = require("./copywriting-agent.service");
...
describe('CopywritingAgentService', () => {
    ...
    beforeEach(async () => {
        ...
        service = module.get(copywriting_agent_service_1.CopywritingAgentService);
        geminiService = module.get(gemini_service_1.GeminiService);
        qwenService = module.get(qwen_service_1.QwenService);
    ...
    describe('禁忌词检查', () => {
        it('当内容包含禁忌词时应标记为不合规', async () => {
            ...
            const result = await service.execute(input);
            expect(result).toBeDefined();
            const forbiddenWordCheck = result.complianceCheck.platformRules.find((rule) => rule.name === '禁忌词检查');
    ...
    describe('品牌色彩应用', () => {
        it('应将品牌色彩添加到配色方案中', async () => {
            ...
            expect(uniqueColors.length).toBeLessThanOrEqual(result.visualSuggestions.colorPalette.length);
        });
    });
});
//# sourceMappingURL=copywriting-agent.service.spec.js.map