...
class TestTenantRepository extends tenant_repository_1.TenantRepository {
    addTenantConditionPublic(queryBuilder) {
        return this.addTenantCondition(queryBuilder);
    }
    getCurrentTenantIdPublic() {
...
describe('TenantRepository', () => {
    ...
    describe('addTenantCondition', () => {
        it('should add tenant condition to query builder', () => {
            ...
            expect(queryBuilder.andWhere).toHaveBeenCalledWith('test.tenantId = :tenantId', { tenantId: 'test-tenant-id' });
    ...
    describe('find', () => {
        it('should add tenant condition when finding entities', async () => {
            ...
            mockQueryBuilder.getMany.mockResolvedValue(foundEntities);
            const result = await testRepository.find();
            expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('test.tenantId = :tenantId', { tenantId: 'test-tenant-id' });
    ...
    describe('findByIds', () => {
        it('should add tenant condition and ids condition when finding by ids', async () => {
            ...
            expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('test.tenantId = :tenantId', { tenantId: 'test-tenant-id' });
            expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('test.id IN (:...ids)', { ids: ['id1', 'id2'] });
            expect(result).toEqual(foundEntities);
        });
    });
    describe('count', () => {
    ...
    describe('getTenantStats', () => {
        it('should return tenant statistics', async () => {
            ...
            expect(result.sizeEstimate).toBe(10 * 1024);
        });
