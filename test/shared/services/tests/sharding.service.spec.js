...
const sharding_service_1 = require("../../../../src/shared/services/sharding.service");
describe('ShardingService', () => {
    ...
    beforeEach(async () => {
        ...
        const module = await testing_1.Test.createTestingModule({
            providers: [
                sharding_service_1.ShardingService,
                { provide: typeorm_1.Connection, useValue: mockConnection },
            ],
        }).compile();
        service = module.get(sharding_service_1.ShardingService);
    });
    describe('initializePartitions', () => {
        it('should initialize partitions for default tables', async () => {
            mockConnection.query.mockResolvedValueOnce([{ table_count: 1 }]);
            mockConnection.query.mockResolvedValueOnce([{ partition_count: 0 }]);
            mockConnection.query.mockResolvedValueOnce([{ column_count: 1 }]);
    ...
    describe('isTablePartitioned', () => {
        it('should return true when table is partitioned', async () => {
            ...
            expect(mockConnection.query).toHaveBeenCalledWith(expect.stringContaining('SELECT COUNT(*) as partition_count'), ['customer_profiles']);
        });
    ...
    describe('getPartitionInfo', () => {
        it('should return partition information', async () => {
            ...
            expect(mockConnection.query).toHaveBeenCalledWith(expect.stringContaining('information_schema.partitions'), ['customer_profiles']);
        });
    });
    describe('analyzePartitionBalance', () => {
        ...
        it('should detect imbalances', async () => {
            const mockPartitions = [
                ...
                { partition_name: 'p3', table_rows: 1000 },
            ];
            mockConnection.query.mockResolvedValueOnce(mockPartitions);
            const result = await service.analyzePartitionBalance('customer_profiles');
    ...
    describe('getTenantDistribution', () => {
        it('should return tenant distribution', async () => {
            ...
            expect(mockConnection.query).toHaveBeenCalledWith(expect.stringContaining('tenant_id'), ['customer_profiles', 'customer_profiles', 20]);
        });
    });
    ...
    describe('generateMigrationReport', () => {
        it('should generate migration report for non-partitioned table', async () => {
            mockConnection.query.mockResolvedValueOnce([{ table_count: 1 }]);
            mockConnection.query.mockResolvedValueOnce([{ partition_count: 0 }]);
            mockConnection.query.mockResolvedValueOnce([{ count: 500000 }]);
        ...
        it('should generate migration report for partitioned table', async () => {
            mockConnection.query.mockResolvedValueOnce([{ table_count: 1 }]);
            mockConnection.query.mockResolvedValueOnce([{ partition_count: 16 }]);
            mockConnection.query.mockResolvedValueOnce([{ count: 500000 }]);
            const result = await service.generateMigrationReport('customer_profiles');
            expect(result.exists).toBe(true);
            expect(result.isPartitioned).toBe(true);
...
//# sourceMappingURL=sharding.service.spec.js.map