...
describe('MockAdapter', () => {
    let adapter;
    beforeEach(() => {
        adapter = new mock_adapter_1.MockAdapter();
    });
    describe('基础功能', () => {
        it('应该正确返回名称', () => {
    ...
    describe('MockStorageService', () => {
        ...
        describe('downloadFile', () => {
            ...
            it('应该对不存在的文件抛出错误', async () => {
                await expect(adapter.storage.downloadFile(bucket, 'nonexistent.txt')).rejects.toThrow(/文件不存在/);
            });
        });
        describe('deleteFile', () => {
            it('应该成功删除文件', async () => {
                ...
                await expect(adapter.storage.downloadFile(bucket, key)).rejects.toThrow(/文件不存在/);
            });
            it('应该安全地删除不存在的文件', async () => {
                await expect(adapter.storage.deleteFile(bucket, 'nonexistent.txt')).resolves.not.toThrow();
            });
        });
        describe('getFileUrl', () => {
        ...
        describe('listFiles', () => {
            ...
            it('应该列出指定bucket中的所有文件', async () => {
                ...
                expect(files.map((f) => f.key)).toEqual(expect.arrayContaining(['file1.txt', 'folder/file2.txt']));
            });
            it('应该支持前缀过滤', async () => {
    ...
    describe('MockAIService', () => {
        ...
        describe('listAvailableModels', () => {
            it('应该返回模型列表', async () => {
                ...
                expect(geminiModel?.capabilities).toEqual(expect.arrayContaining([
                    'text-generation',
                    'code-generation',
                    'translation',
                ]));
    ...
    describe('MockDatabaseService', () => {
        ...
        describe('query', () => {
            it('应该执行查询并返回空数组', async () => {
                const result = await adapter.database.query(sql, params);
        ...
        describe('sharding', () => {
            ...
            describe('migrateData', () => {
                it('应该模拟数据迁移', async () => {
                    ...
                    expect(result.migratedRows).toBeLessThanOrEqual(1099);
    ...
    describe('MockMessagingService', () => {
        ...
        beforeEach(() => {
            adapter.messaging.queues.clear();
            adapter.messaging.subscriptions.clear();
        });
        describe('sendMessage', () => {
            ...
            it('应该创建队列如果不存在', async () => {
                ...
                const queues = adapter.messaging.queues;
        ...
        describe('acknowledgeMessage', () => {
            it('应该成功确认消息', async () => {
                const messageId = await adapter.messaging.sendMessage(queue, message);
                await expect(adapter.messaging.acknowledgeMessage(queue, messageId)).resolves.not.toThrow();
            });
            it('应该安全地确认不存在的消息', async () => {
                await expect(adapter.messaging.acknowledgeMessage(queue, 'nonexistent-id')).resolves.not.toThrow();
            });
        });
        describe('publishEvent', () => {
            it('应该发布事件到主题', async () => {
                const event = { type: 'user.created', userId: '123' };
                await expect(adapter.messaging.publishEvent(topic, event)).resolves.not.toThrow();
            });
        ...
        describe('subscribeEvent', () => {
            ...
            it('应该允许多次订阅同一主题', async () => {
                ...
                await adapter.messaging.subscribeEvent(topic, handler1);
                await adapter.messaging.subscribeEvent(topic, handler2);
                const subscriptions = adapter.messaging.subscriptions.get(topic);
                expect(subscriptions).toHaveLength(2);
            });
        });
    });
});
//# sourceMappingURL=mock.adapter.spec.js.map