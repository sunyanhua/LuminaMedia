...
jest.mock('../../../src/shared/cloud/adapters/mock.adapter', () => ({
...
jest.mock('../../../src/shared/cloud/adapters/alicloud.adapter', () => ({
...
jest.mock('../../../src/shared/cloud/adapters/private-deploy.adapter', () => ({
...
describe('CloudProviderFactory', () => {
    ...
    describe('getInstance', () => {
        ...
        it('应该返回PrivateDeployAdapter当CLOUD_PROVIDER设置为private', async () => {
            ...
            const { PrivateDeployAdapter, } = require('../../../src/shared/cloud/adapters/private-deploy.adapter');
        ...
        it('应该实现单例模式，多次调用返回相同实例', async () => {
            const provider1 = await cloud_provider_factory_1.CloudProviderFactory.getInstance();
            const provider2 = await cloud_provider_factory_1.CloudProviderFactory.getInstance();
    ...
    describe('destroyInstance', () => {
        it('应该清理当前实例并允许创建新实例', async () => {
            ...
            expect(cloud_provider_factory_1.CloudProviderFactory['instance']).toBeNull();
            const provider2 = await cloud_provider_factory_1.CloudProviderFactory.getInstance();
        ...
        it('应该安全地调用当没有实例存在时', () => {
            expect(cloud_provider_factory_1.CloudProviderFactory['instance']).toBeNull();
            expect(() => cloud_provider_factory_1.CloudProviderFactory.destroyInstance()).not.toThrow();
        });
    });
    describe('便捷函数', () => {
        describe('getCloudProvider', () => {
            it('应该返回与CloudProviderFactory.getInstance相同的实例', async () => {
                const provider1 = await (0, cloud_provider_factory_1.getCloudProvider)();
                const provider2 = await cloud_provider_factory_1.CloudProviderFactory.getInstance();
                expect(provider1).toBe(provider2);
            });
        });
        describe('getStorageService', () => {
            it('应该返回provider的storage服务', async () => {
                const provider = await cloud_provider_factory_1.CloudProviderFactory.getInstance();
                const storageService = await (0, cloud_provider_factory_1.getStorageService)();
        ...
        describe('getAIService', () => {
            it('应该返回provider的ai服务', async () => {
                const provider = await cloud_provider_factory_1.CloudProviderFactory.getInstance();
                const aiService = await (0, cloud_provider_factory_1.getAIService)();
        ...
        describe('getDatabaseService', () => {
            it('应该返回provider的database服务', async () => {
                const provider = await cloud_provider_factory_1.CloudProviderFactory.getInstance();
                const databaseService = await (0, cloud_provider_factory_1.getDatabaseService)();
        ...
        describe('getMessagingService', () => {
            it('应该返回provider的messaging服务', async () => {
                const provider = await cloud_provider_factory_1.CloudProviderFactory.getInstance();
                const messagingService = await (0, cloud_provider_factory_1.getMessagingService)();
...
//# sourceMappingURL=cloud-provider.factory.spec.js.map