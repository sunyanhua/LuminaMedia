...
let GeminiService = GeminiService_1 = class GeminiService {
    ...
    async initializeWithCurrentKey() {
        ...
        try {
            this.config = {
                ...
                temperature: this.configService.get('GEMINI_TEMPERATURE', 0.8),
                maxTokens: this.configService.get('GEMINI_MAX_TOKENS', 8192),
    ...
    async initialize() {
        ...
        console.log('>>> [DOCKER ENV DEBUG] Key length:', this.configService.get('GEMINI_API_KEY', '')?.length || 0);
        ...
        const apiKeyString = this.configService.get('GEMINI_API_KEY', '');
        this.apiKeys = this.parseApiKeys(apiKeyString);
        if (this.apiKeys.length === 0) {
            this.logger.warn('未配置有效的Gemini API Key。GeminiService将使用回退模式。');
        ...
        const rotationMode = this.configService.get('GEMINI_KEY_ROTATION', 'sequential');
    ...
    async checkGeminiHealth(retryCount = 0) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                this.recordKeyFailure(currentKey);
                const isInvalidKeyError = response.status === 400 ||
                    response.status === 403 ||
            ...
            const modelExists = data.models.some((m) => m.name === `models/${configuredModel}` ||
        ...
        if (result.text) {
            ...
            if (parsedResponse.success) {
                const dataWithEngine = {
                    ...parsedResponse.data,
                    engine: gemini_interface_1.AIEngine.GEMINI,
                };
                return {
                    success: true,
                    data: dataWithEngine,
...
7. 只返回JSON，不包含任何解释性文字`;
    ...
    ensureJsonClosed(raw) {
        ...
        const hasSuggestedImages = text.includes('"suggestedImages"') ||
    ...
    async generateContent(options) {
        ...
        const { prompt, platform, tone = 'casual', wordCount, includeHashtags = true, includeImageSuggestions = true, } = options;
        console.log(`>>> [generateContent] isGeminiAvailable: ${this.isGeminiAvailable()}`);
        if (!this.isGeminiAvailable()) {
            this.logger.warn('Gemini API not available, trying REST API for content generation');
            const result = await this.generateContentViaRest(prompt, {
        ...
        const contentPrompt = this.buildContentPrompt(prompt, platform, tone, wordCount);
    ...
    async generateMarketingContent(options) {
        const { campaignSummary, targetPlatforms, contentTypes = ['promotional'], tone = 'professional', quantity = 1, } = options;
        if (!this.isGeminiAvailable()) {
            this.logger.warn('Gemini API not available, using fallback marketing content generation');
        ...
        try {
            ...
            for (const platform of targetPlatforms) {
                for (let i = 0; i < quantity; i++) {
                    const prompt = this.buildMarketingContentPrompt(campaignSummary, platform, contentTypes, tone);
            ...
            if (contents.length === 0) {
                throw new Error('Failed to generate any content for the marketing campaign');
            }
            const overallQualityScore = contents.reduce((sum, content) => sum + (content.qualityScore || 50), 0) / contents.length;
            ...
            const marketingContent = {
                ...
                contentStrategySummary: this.generateContentStrategySummary(contents, campaignSummary),
    ...
    buildContentPrompt(prompt, platform, tone, wordCount) {
...
输出要求：仅返回JSON对象，不要包含任何额外文本或Markdown代码块。确保JSON结构完整闭合。`;
    ...
    buildMarketingContentPrompt(campaignSummary, platform, contentTypes, tone) {
        const platformInstructions = this.getPlatformInstructions(platform);
...
输出要求：仅返回JSON对象，不要包含任何额外文本或Markdown代码块。确保JSON结构完整闭合。`;
    ...
    parseContentResponse(text, platform, options) {
        try {
            let rawText = text;
            rawText = rawText
                .replace(/```json/g, '')
            ...
            const estimatedReadingTime = readingMinutes === 1 ? '1分钟' : `${readingMinutes}分钟`;
            return {
    ...
    assessContentQuality(content, platform) {
        ...
        return {
            ...
            improvementSuggestions: this.generateImprovementSuggestions(score, content, platform),
        };
    }
    getPlatformFitScore(content, platform) {
        switch (platform) {
            case platform_enum_1.Platform.XHS:
                const hasEmoji = /[\uD800-\uDBFF][\uDC00-\uDFFF]/.test(content.content);
                const hasShortParagraphs = (content.content.match(/\n\n/g) || []).length > 2;
            ...
            case platform_enum_1.Platform.DOUYIN:
                const hasHotTopic = /#(挑战|热门|话题)/.test(content.content);
                const isConcise = (content.wordCount || 0) < 200;
                const hasInteractionCall = /(关注|点赞|评论|分享)/.test(content.content);
                return ((hasHotTopic ? 10 : 0) +
                    (isConcise ? 10 : 0) +
    ...
    generateQualityFeedback(score, content) {
        if (score >= 85) {
    ...
    generateImprovementSuggestions(score, content, platform) {
        const suggestions = [];
    ...
    getBestPostingTimes(platform) {
        switch (platform) {
            ...
            case platform_enum_1.Platform.DOUYIN:
                return ['12:00-14:00', '18:00-20:00', '21:00-23:00'];
            default:
                return ['09:00-11:00', '14:00-16:00', '19:00-21:00'];
        }
    }
    getRecommendedFrequency(platform) {
        switch (platform) {
            case platform_enum_1.Platform.XHS:
                return '每周3-5次';
            case platform_enum_1.Platform.WECHAT_MP:
    ...
    calculateConsistencyScore(contents) {
    ...
    generateContentStrategySummary(contents, campaignSummary) {
        const platformCount = new Set(contents.map((c) => c.platform)).size;
        const totalContentCount = contents.length;
        const avgQualityScore = contents.reduce((sum, c) => sum + (c.qualityScore || 50), 0) /
            contents.length;
