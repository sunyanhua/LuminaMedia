...
const marketing_strategy_service_1 = require("./marketing-strategy.service");
...
let DemoService = DemoService_1 = class DemoService {
    ...
    logger = new common_1.Logger(DemoService_1.name);
    constructor(customerProfileRepository, customerSegmentRepository, dataImportJobRepository, marketingCampaignRepository, marketingStrategyRepository, userBehaviorRepository, contentGenerationService, marketingStrategyService, tenantContextService) {
    ...
    async createMallCustomerDemo(userId = 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11') {
        ...
        try {
            ...
            const campaign = await this.createDemoCampaign(userId, customerProfile.id);
            const strategies = await this.generateDemoStrategies(campaign.id);
            let contentGenerationResult = null;
            if (strategies.length > 0) {
                contentGenerationResult = await this.generateDemoContent(campaign, strategies[0]);
            }
            return {
                customerProfile,
                segments,
    ...
    async createDemoCustomerProfile(userId) {
        const tenantId = this.tenantContextService.getCurrentTenantId();
        const profileData = {
            customerName: '商场顾客数据',
            customerType: customer_type_enum_1.CustomerType.INDIVIDUAL,
            industry: industry_enum_1.Industry.RETAIL,
            tenantId,
            dataSources: [
                {
                    type: 'CSV',
            ...
            profileData: {
                ...
                description: '基于1000条商场顾客消费记录的模拟数据，包含详细的人口统计、消费习惯和偏好分析',
            },
            behaviorInsights: {
    ...
    async createDemoSegments(customerProfileId) {
        const tenantId = this.tenantContextService.getCurrentTenantId();
        const segmentsData = [
            {
    ...
    async createDemoCampaign(userId, customerProfileId) {
        const tenantId = this.tenantContextService.getCurrentTenantId();
        const campaignData = {
            tenantId,
            userId,
    ...
    async generateDemoStrategies(campaignId) {
        const strategyTypes = [
        ...
        for (const strategyType of strategyTypes) {
            try {
                const result = await this.marketingStrategyService.generateStrategy(campaignId, strategyType, generation_method_enum_1.GenerationMethod.AI_GENERATED, true);
                strategies.push(result.strategy);
            }
            catch (error) {
                this.logger.warn(`Failed to generate strategy type ${strategyType}: ${error.message}`);
                const fallbackStrategy = await this.createFallbackStrategy(campaignId, strategyType);
                strategies.push(fallbackStrategy);
            }
        }
        return strategies;
    }
    async createFallbackStrategy(campaignId, strategyType) {
        const tenantId = this.tenantContextService.getCurrentTenantId();
        const strategyData = {
            tenantId,
            campaignId,
            ...
            generatedBy: generation_method_enum_1.GenerationMethod.TEMPLATE,
        };
    ...
    async generateDemoContent(campaign, strategy) {
        try {
            ...
            const result = await this.contentGenerationService.generateMarketingContent({
                ...
                targetPlatforms: [platform_enum_1.Platform.XHS, platform_enum_1.Platform.WECHAT_MP],
                contentTypes: ['promotional', 'educational'],
                tone: 'friendly',
                quantity: 2,
            });
            if (result.success && result.marketingContent) {
                strategy.generatedContentIds = result.marketingContent.contents.map((c) => c.title);
                strategy.contentPlatforms =
                    result.marketingContent.recommendedPostingSchedule.map((s) => s.platform);
    ...
    async resetDemoData(userId = 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11') {
        ...
        if (profileIds.length > 0) {
            const orphanStrategies = await this.marketingStrategyRepository
                ...
                .where('strategy.campaignId IS NULL AND strategy.customerProfileId IN (:...profileIds) AND strategy.tenantId = :tenantId', { profileIds, tenantId })
                .getMany();
    ...
    async getDemoProgress(userId = 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11') {
        try {
            const tenantId = this.tenantContextService.getCurrentTenantId();
    ...
    async getDemoResults(userId = 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11') {
        try {
            const tenantId = this.tenantContextService.getCurrentTenantId();
            ...
            const totalDataPoints = (profile?.profileData?.totalCustomers || 0) +
                segments.length +
                strategies.length;
            const analysisCompleted = !!profile;
            const strategiesGenerated = strategies.length;
            const contentGenerated = strategies.some((s) => s.generatedContentIds && s.generatedContentIds.length > 0);
            let overallScore = 0;
    ...
    async validateDemoResults(userId = 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11') {
        try {
    ...
    async generateDemoReport(userId = 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11') {
        try {
            const results = await this.getDemoResults(userId);
    ...
    generateHtmlReport(results, validation) {
...
<body>
    ...
    <div class="section">
        ...
        <h3>反馈:</h3>
        <ul>
            ${validation.feedback.map((item) => `<li>${item}</li>`).join('')}
        </ul>
        <h3>改进建议:</h3>
        <ul>
            ${validation.improvements.map((item) => `<li>${item}</li>`).join('')}
        </ul>
    </div>
    ...
    async executeDemoStep(step, userId = 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11', stepData) {
        try {
            ...
            let result = null;
            let message = '步骤执行成功';
            const nextStep = step < 6 ? step + 1 : undefined;
            const tenantId = this.tenantContextService.getCurrentTenantId();
            ...
            switch (step) {
                ...
                case 5:
                    ...
                    const campaignForStrategies = await this.marketingCampaignRepository.findOne({
                        where: {
                            userId,
                    ...
                    const strategies = await this.generateDemoStrategies(campaignForStrategies.id);
                    result = strategies;
                ...
                case 6:
                    ...
                    const campaignForContent = await this.marketingCampaignRepository.findOne({
                        where: {
                            userId,
                            customerProfileId: customerProfile.id,
                            tenantId,
                    ...
                    const strategiesForContent = await this.marketingStrategyRepository.find({
                    ...
                    const contentResult = await this.generateDemoContent(campaignForContent, strategiesForContent[0]);
            ...
            this.logger.error(`Failed to execute demo step ${step}: ${error.message}`);
            return {
                success: false,
                step,
                stepName: '步骤执行失败',
...
exports.DemoService = DemoService = DemoService_1 = __decorate([
    ...
    __param(1, (0, typeorm_1.InjectRepository)(customer_segment_repository_1.CustomerSegmentRepository)),
    __param(2, (0, typeorm_1.InjectRepository)(data_import_job_repository_1.DataImportJobRepository)),
    __param(5, (0, typeorm_1.InjectRepository)(user_behavior_repository_1.UserBehaviorRepository)),
