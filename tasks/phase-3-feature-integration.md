...
### 任务1：控制器功能开关集成
- [x] 为 `src/modules/analytics/controllers/customer-analytics.controller.ts` 添加 @Feature() 装饰器
  - 标记 `customer-analytics` 功能
  - 配置所需权限
  - 配置 FeatureGuard 和 JwtAuthGuard
- [x] 为 `src/modules/government/controllers/government-content.controller.ts` 添加 @Feature() 装饰器
  - 标记 `government-publish` 功能
  - 配置所需权限
  - 配置 FeatureGuard 和 JwtAuthGuard
- [x] 为 `src/modules/government/controllers/sentiment-analysis.controller.ts` 添加 @Feature() 装饰器
  - 标记 `sentiment-analysis` 功能
  - 配置所需权限
  - 配置 FeatureGuard 和 JwtAuthGuard
- [x] 为 `src/modules/monitor/controllers/geo-analysis.controller.ts` 添加 @Feature() 装饰器
  - 标记 `geo-analysis` 功能
  - 配置所需权限
  - 配置 FeatureGuard 和 JwtAuthGuard
- [x] 为 `src/modules/publish/controllers/account.controller.ts` 添加 @Feature() 装饰器
  - 标记 `matrix-publish` 功能
  - 配置所需权限
  - 配置 FeatureGuard 和 JwtAuthGuard

### 任务2：功能配置管理API开发
- [ ] 创建 `src/modules/auth/controllers/feature-config.controller.ts`
...
### 任务3：租户功能管理API开发
- [ ] 创建 `src/modules/auth/controllers/tenant-feature.controller.ts`
...
### 任务4：配额管理API开发
- [ ] 创建 `src/modules/auth/controllers/quota.controller.ts`
...
### 任务5：演示数据管理API开发
- [ ] 更新 `src/modules/analytics/controllers/demo.controller.ts`
...
- [ ] 更新 `src/modules/auth/auth.module.ts`
  ...
  - 注册 QuotaController
- [ ] 更新 `src/modules/analytics/analytics.module.ts`
