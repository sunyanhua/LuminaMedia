...
import { Users, TrendingUp, Clock, Activity, TriangleAlert as AlertTriangle } from 'lucide-react';
...
function BusinessAnalytics() {
  ...
  const [activityData, setActivityData] = useState<ParkingSpendingData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 获取用户活跃度与消费关系数据（复用停车消费数据接口）
        const activityResponse = await analyticsService.getParkingSpendingData('demo');
        ...
        console.error('Failed to fetch business analytics data:', err);
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-slate-100 mb-2">企业数据分析</h2>
            <p className="text-slate-400">深度洞察客户行为和业务转化模式</p>
          </div>
  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="bg-slate-900 border-slate-800">
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm text-slate-400">今日活跃用户</p>
                <h3 className="text-2xl font-bold text-slate-100">{formatNumber(8945)}</h3>
                <p className="text-xs text-slate-500 mt-1">较昨日 +12.5%</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">较昨日 +3.2%</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-slate-900 border-slate-800">
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm text-slate-400">平均会话时长</p>
                <h3 className="text-2xl font-bold text-slate-100">2.4 小时</h3>
                <p className="text-xs text-slate-500 mt-1">较昨日 -5.1%</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card className="bg-slate-900 border-slate-800">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-slate-100">客户分群画像</CardTitle>
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-3 gap-4 text-sm">
                      <div>
                        <p className="text-slate-500 mb-1">客户数</p>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500">API数据</p>
                      </div>
                      <div>
                        <p className="text-slate-500 mb-1">活跃频次</p>
                        <p className="font-semibold text-slate-200">{persona.description || '每周1-2次'}</p>
                      </div>
                    </div>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="bg-slate-900 border-slate-800">
          <CardHeader>
            <CardTitle className="text-slate-100">用户活跃度 vs. 转化金额</CardTitle>
            ...
            <div className="h-64">
              <ChartContainer config={activityChartConfig}>
                ...
                >
                  ...
                  <Bar
                    dataKey="avgSpending"
                    fill={activityChartConfig.avgSpending.color}
                    radius={[4, 4, 0, 0]}
                  ...
                  <Bar
                    dataKey="userCount"
                    fill={activityChartConfig.userCount.color}
                    radius={[4, 4, 0, 0]}
                    className="hover:opacity-80 transition-opacity"
                    name="用户数"
                  />
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-300">
                    高活跃度用户平均消费高出75%。建议实施会员忠诚度计划提升用户粘性。
                  </p>
                </div>
              </div>
            </div>
        ...
        <Card className="bg-slate-900 border-slate-800">
          <CardHeader>
            <CardTitle className="text-slate-100">每日活跃用户趋势（{timeRange}天）</CardTitle>
            <p className="text-sm text-slate-400">高峰时段和活跃趋势</p>
          </CardHeader>
          <CardContent>
            <div className="h-64">
              <ChartContainer config={trafficChartConfig}>
                ...
                >
                  ...
                  <Tooltip
                    ...
                    labelStyle={{ color: '#cbd5e1' }}
                    formatter={(value: number) => [formatNumber(value), '活跃用户数']}
