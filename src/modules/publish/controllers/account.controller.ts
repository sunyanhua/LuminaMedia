...
export class AccountController {
   ...
   */
  @Get()
  @ApiOperation({ summary: '获取所有账号列表', description: '返回所有社交媒体账号的基本信息（不包含凭证）' })
  @ApiQuery({ name: 'tenantId', required: false, description: '租户ID，默认为demo-tenant' })
  ...
  @ApiOperation({ summary: '创建或更新账号凭证', description: '加密存储社交媒体账号凭证' })
  ...
  async createOrUpdateAccount(
    ...
    const account = await this.accountCredentialService.encryptAndStoreCredentials(
      accountId,
      platform,
      credentials,
      tenantId,
   ...
   */
  @Get(':accountId')
  @ApiOperation({ summary: '获取账号详情', description: '获取指定账号的详细信息（不包含解密后的凭证）' })
  @ApiParam({ name: 'accountId', description: '账号ID' })
  @ApiQuery({ name: 'tenantId', required: false, description: '租户ID，默认为demo-tenant' })
   ...
   */
  @Delete(':accountId')
  @ApiOperation({ summary: '删除账号凭证', description: '删除指定账号的凭证（标记为过期）' })
  @ApiParam({ name: 'accountId', description: '账号ID' })
  @ApiQuery({ name: 'tenantId', required: false, description: '租户ID，默认为demo-tenant' })
   ...
   */
  @Post(':accountId/test')
  @ApiOperation({ summary: '测试账号连接', description: '测试指定社交媒体账号的连接状态' })
  @ApiParam({ name: 'accountId', description: '账号ID' })
  @ApiQuery({ name: 'tenantId', required: false, description: '租户ID，默认为demo-tenant' })
   ...
   */
  @Post('test/all')
  @ApiOperation({ summary: '测试所有账号连接', description: '批量测试所有社交媒体账号的连接状态' })
  @ApiQuery({ name: 'tenantId', required: false, description: '租户ID，默认为demo-tenant' })
  ...
  @ApiOperation({ summary: '验证账号凭证', description: '验证指定账号凭证的有效性' })
  @ApiParam({ name: 'accountId', description: '账号ID' })
  @ApiQuery({ name: 'tenantId', required: false, description: '租户ID，默认为demo-tenant' })
  ...
  @ApiOperation({ summary: '更新账号配置', description: '更新社交媒体账号的配置信息' })
  @ApiParam({ name: 'accountId', description: '账号ID' })
  @ApiQuery({ name: 'tenantId', required: false, description: '租户ID，默认为demo-tenant' })
