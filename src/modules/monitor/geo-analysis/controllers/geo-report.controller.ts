...
export class GeoReportController {
  ...
  async exportReport(
    ...
    if (format === 'pdf') {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="geo-analysis-${analysisId}.pdf"`,
      );
      // 实际应调用PDF生成服务
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="geo-analysis-${analysisId}.json"`,
      );
      res.send(JSON.stringify(result, null, 2));
    }
  }
  ...
  private generateSummary(result: any): string {
    ...
    if (competitiveAnalysis) {
      ...
      const competitorCount =
        competitiveAnalysis.competitorAnalysis?.competitors?.length || 0;
