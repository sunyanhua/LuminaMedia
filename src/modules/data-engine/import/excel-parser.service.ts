  ...
  async detectHeaderMapping(
    ...
    const targetPatterns =
      targetFields.length > 0
        ? Object.fromEntries(
            Object.entries(fieldPatterns).filter(([key]) =>
    ...
    for (const header of headers) {
      ...
      if (!matched) {
        for (const [targetField, patterns] of Object.entries(targetPatterns)) {
          ...
          if (
            keywords.some(
              (keyword) => headerLower.includes(keyword) && keyword.length > 1,
            )
          ) {
            mapping[header] = targetField;
            break;
    ...
    // 计算匹配准确率
    const matchRate =
      headers.length > 0 ? Object.keys(mapping).length / headers.length : 0;
    console.log(
      `表头匹配完成: ${Object.keys(mapping).length}/${headers.length} (${(matchRate * 100).toFixed(1)}%)`,
    );
