...
import { StrategyAgentInput } from '../interfaces/strategy-agent.interface';
...
describe('StrategyAgentService', () => {
  ...
  describe('execute', () => {
    it('当AI引擎可用时应成功执行策划', async () => {
      const analysisResults: AnalysisAgentOutput = {
        marketInsights: {
    ...
    it('当AI引擎不可用时应返回回退策划', async () => {
      ...
      expect(result.budgetPlan.totalBudget).toBe(
        input.budgetConstraints.totalBudget,
      );
      // 回退策划不应调用AI生成
  ...
  describe('parseStrategyResponse', () => {
    ...
    it('当AI响应缺少必需字段时应补充默认值', async () => {
      const partialResponse = JSON.stringify({
        ...
        // 缺少 successMetrics 和 riskAssessment
      });
  ...
  describe('adjustStrategyWithConstraints', () => {
    it('应根据预算约束调整策划方案', async () => {
      ...
      expect(result.budgetPlan.breakdown[0].amount).toBe(50000 * 0.3); // 内容制作占30%
