 ...
 * 标准营销策划模板 - 适用于通用营销活动
 */
export const standardStrategyTemplate: StrategyPromptTemplate = {
  id: 'standard',
  name: '标准营销策划模板',
  scenarios: ['品牌推广', '产品发布', '促销活动', '用户增长'],
  campaignTypes: ['综合型', '品牌型', '促销型', '内容型'],
  depthLevel: 3,
  generatePrompt: (params: StrategyPromptParams) => {
    const {
      analysisResults,
      currentEvents,
      holidays,
 ...
 * 新品发布专项模板
 */
export const productLaunchTemplate: StrategyPromptTemplate = {
  id: 'product_launch',
  name: '新品发布专项模板',
 ...
 * 品牌建设专项模板
 */
export const brandBuildingTemplate: StrategyPromptTemplate = {
  id: 'brand_building',
  name: '品牌建设专项模板',
 ...
 * 促销转化专项模板
 */
export const salesPromotionTemplate: StrategyPromptTemplate = {
  id: 'sales_promotion',
  name: '促销转化专项模板',
 ...
 * 生成分析结果摘要
 */
function generateAnalysisSummary(analysis: AnalysisAgentOutput): string {
  const {
    marketInsights,
    targetAudience,
    competitorAnalysis,
 ...
 * 模板选择器
 */
export function selectStrategyTemplate(
  scenario: string,
  campaignType: string,
  ...
  // 按场景和类型匹配
  if (
    ['产品发布', '新品发布', '功能发布', '版本发布'].some((keyword) =>
      scenarioLower.includes(keyword),
    )
  ) {
  ...
  if (
    ['品牌建设', '品牌升级', '品牌重塑', '品牌年轻化'].some((keyword) =>
      scenarioLower.includes(keyword),
    )
  ) {
    return brandBuildingTemplate;
  }

  if (
    ['促销', '销售提升', '转化', '清仓'].some((keyword) =>
      scenarioLower.includes(keyword),
    )
  ) {
  ...
  // 默认返回标准模板
  return standardStrategyTemplate;
}

/**
 * 所有可用模板
 */
export const allStrategyTemplates: StrategyPromptTemplate[] = [
  standardStrategyTemplate,
  productLaunchTemplate,
  brandBuildingTemplate,
