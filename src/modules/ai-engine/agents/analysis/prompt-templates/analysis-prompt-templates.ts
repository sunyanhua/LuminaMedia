 ...
 * 标准分析模板 - 适用于大多数行业
 */
export const standardAnalysisTemplate: AnalysisPromptTemplate = {
  id: 'standard',
  name: '标准市场分析模板',
  industries: ['通用'],
  depthLevel: 3,
  generatePrompt: (params: AnalysisPromptParams) => {
    const { industryContext, businessGoals, profileSummary, knowledgeContext } =
      params;
...
/**
 * 深度行业分析模板 - 适用于需要深度行业洞察的场景
 */
export const deepIndustryAnalysisTemplate: AnalysisPromptTemplate = {
  id: 'deep_industry',
  name: '深度行业分析模板',
 ...
 * 快速扫描模板 - 适用于快速初步分析
 */
export const quickScanTemplate: AnalysisPromptTemplate = {
  id: 'quick_scan',
  name: '快速市场扫描模板',
  industries: ['通用'],
  depthLevel: 1,
 ...
 * 电商零售行业专用模板
 */
export const ecommerceAnalysisTemplate: AnalysisPromptTemplate = {
  id: 'ecommerce',
  name: '电商零售分析模板',
 ...
 * B2B企业服务行业模板
 */
export const b2bAnalysisTemplate: AnalysisPromptTemplate = {
  id: 'b2b',
  name: 'B2B企业服务分析模板',
...
export function selectTemplate(
  industry: string,
  depthRequired: number = 3,
): AnalysisPromptTemplate {
  const industryLower = industry.toLowerCase();

  // 按行业匹配
  if (
    ['电商', '零售', '消费品', 'ecommerce'].some((keyword) =>
      industryLower.includes(keyword),
    )
  ) {
    return ecommerceAnalysisTemplate;
  }

  if (
    ['企业服务', 'saas', '软件', '咨询', 'b2b'].some((keyword) =>
      industryLower.includes(keyword),
    )
  ) {
    return b2bAnalysisTemplate;
  }

  if (
    ['金融', '医疗', '教育'].some((keyword) => industryLower.includes(keyword))
  ) {
    return deepIndustryAnalysisTemplate;
  }
  ...
  // 默认返回标准模板
  return standardAnalysisTemplate;
}

/**
 * 所有可用模板
 */
export const allTemplates: AnalysisPromptTemplate[] = [
  standardAnalysisTemplate,
  deepIndustryAnalysisTemplate,
  quickScanTemplate,
  ecommerceAnalysisTemplate,
  b2bAnalysisTemplate,
];
