 ...
 * 标准多平台文案模板 - 适用于通用营销活动
 */
export const standardMultiPlatformTemplate: CopywritingPromptTemplate = {
  id: 'standard_multi_platform',
  name: '标准多平台文案模板',
  platforms: ['wechat', 'xiaohongshu', 'weibo', 'douyin'],
  contentType: 'campaign',
  depthLevel: 3,
  generatePrompt: (params: CopywritingPromptParams) => {
    const {
      strategyPlan,
      platformSpecs,
      brandGuidelines,
 ...
 * 品牌宣传专项模板
 */
export const brandStorytellingTemplate: CopywritingPromptTemplate = {
 ...
 * 产品推广专项模板
 */
export const productPromotionTemplate: CopywritingPromptTemplate = {
 ...
 * 节日营销专项模板
 */
export const holidayMarketingTemplate: CopywritingPromptTemplate = {
 ...
 * 危机公关/品牌声誉模板
 */
export const crisisCommunicationTemplate: CopywritingPromptTemplate = {
  ...
  generatePrompt: (params: CopywritingPromptParams) => {
    ...
    const situationDescription =
      customParams?.situationDescription || '具体情况描述';
 ...
 * 生成策略摘要
 */
function generateStrategySummary(strategy: StrategyAgentOutput): string {
  const { campaignTheme, marketingStrategy, activityPlan, budgetPlan } =
    strategy;
...
${activityPlan.timeline
  ...
  .map(
    (item) =>
      `- 第${item.weekNumber}周：${item.keyActivities.slice(0, 2).join('、')}`,
  )
  .join('\n')}
 ...
 * 模板选择器
 */
export function selectCopywritingTemplate(
  platforms: ('wechat' | 'xiaohongshu' | 'weibo' | 'douyin')[],
  contentType: string,
  scenario?: string,
  ...
  // 按场景匹配
  if (
    scenarioLower.includes('品牌故事') ||
    scenarioLower.includes('品牌建设')
  ) {
    return brandStorytellingTemplate;
  }

  if (
    scenarioLower.includes('产品推广') ||
  ...
  if (scenarioLower.includes('节日') || scenarioLower.includes('节庆')) {
    return holidayMarketingTemplate;
  }

  if (scenarioLower.includes('危机') || scenarioLower.includes('声誉')) {
    return crisisCommunicationTemplate;
  }

  // 按平台数量匹配
  if (platforms.length >= 3) {
    return standardMultiPlatformTemplate;
  }
 ...
 * 所有可用模板
 */
export const allCopywritingTemplates: CopywritingPromptTemplate[] = [
  standardMultiPlatformTemplate,
  brandStorytellingTemplate,
  productPromotionTemplate,
  holidayMarketingTemplate,
